...
def compute_advantage(
    ...
    print(f"我正在准备算优势！当前 estimator = {adv_estimator}")
    ...
    # gdpo 的 advantage 计算
    elif str(adv_estimator).lower() == 'gdpo' or adv_estimator == 'gdpo':
        print(f"我正在准备算优势！当前 estimator = {adv_estimator}")
        grpo_calculation_mask = data.batch["response_mask"]
        index = data.non_tensor_batch["uid"]

        # 提取在 dapo.py 中解耦的奖励
        scores_correctness = data.batch["token_level_scores_correctness"]
        scores_length = data.batch["token_level_scores_length"]
        # scores_prm = data.batch["token_level_scores_prm"]
        if "rm_scores" in data.batch:
            scores_prm = data.batch["rm_scores"]
            #if torch.distributed.get_rank() == 0:
            print(f"🎯 [GDPO 验资成功] 从 GPU 拿到了 rm_scores! 当前 Batch 均值: {scores_prm.mean().item():.4f}")
        else:
            scores_prm = torch.zeros_like(scores_correctness)
            #if torch.distributed.get_rank() == 0:
            print(f"⚠️ [GDPO 验资警告] 依然是空手而归 (拿了 0)！")

        # 分别对 correctness 和 length 进行 Group 级归一化优势计算
        adv_correctness, _ = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=scores_correctness,
            response_mask=grpo_calculation_mask,
            index=index,
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
        )
        
        adv_length, _ = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=scores_length,
            response_mask=grpo_calculation_mask,
            index=index,
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
        )

        scores_prm_clipped = torch.clamp(scores_prm, min=-5.0, max=0.0)
        if torch.all(scores_prm_clipped == 0):
            new_advantage = adv_correctness + adv_length
        else:
            adv_prm, _ = core_algos.compute_grpo_outcome_advantage(
                token_level_rewards=scores_prm_clipped,
                response_mask=grpo_calculation_mask,
                index=index,
                norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
            )
            # 独立归一化后直接相加
            new_advantage = adv_correctness + adv_length + adv_prm

        # 执行 GDPO 的全局 Batch-wise 白化 (白化时忽略 padding)
        advantages = masked_whiten(new_advantage, grpo_calculation_mask) * grpo_calculation_mask

        data.batch["advantages"] = advantages
        data.batch["returns"] = advantages # 无 Value Model 时，returns 占位等于 advantages
